...
import {
  createBuffer, createNullCell, setCellChar, setCellFg, setCellBg,
  CELL_STYLE,
  STYLE_BOLD, STYLE_UNDERLINE, STYLE_ITALIC, STYLE_STRIKETHROUGH,
  STYLE_INVERSE, STYLE_BLINK, STYLE_INVISIBLE,
  STYLE_DIM, STYLE_OVERLINE
...
describe('ANSIGenerator', () => {
  ...
  describe('styles and colors', () => {
    ...
    test('all style flags work correctly', () => {
      ...
      testCases.forEach(({ flag, code }) => {
        ...
        const hasCode = result.includes(`[${code}`) || result.includes(`;${code}`) || result.includes(`;${code};`) || result.includes(`;${code}m`);
        if (!hasCode) {
          console.log(`Failed for flag ${flag.toString(16)} code ${code}, got: ${result}`);
          console.log(`Cell values: [${cell[0]}, 0x${cell[1].toString(16)}, 0x${cell[2].toString(16)}, ${cell[3]}]`);
        }
  ...
  describe('sparse patterns', () => {
    test('diagonal pattern', () => {
      ...
      expect(result).toBe('0\x1b[1B\x1b[1G1\x1b[1B\x1b[2G2\x1b[1B\x1b[3G3\x1b[1B\x1b[4G4');
    });
