...
const bufferGen = (rows: number, cols: number) =>
	fc
		.array(fc.array(cellGen, {minLength: cols, maxLength: cols}), {
			minLength: rows,
			maxLength: rows,
		})
		.map((buffer) => {
...
describe("Delta rendering property tests", () => {
		...
		fc.assert(
			fc.property(
				...
				async (bufferA, bufferB) => {
					...
					const terminalResult = await applyANSIToTerminal(
						bufferA,
						transitionANSI,
						5,
						10,
	...
	test("Empty buffer transitions work correctly", () => {
		fc.assert(
			fc.property(bufferGen(3, 5), async (targetBuffer) => {
	...
	test("Identity transition (buffer to itself) produces no changes", () => {
		fc.assert(
			fc.property(bufferGen(4, 6), async (buffer) => {
...
describe("Resize handling property tests", () => {
	test("Buffer content is preserved when resizing larger", () => {
		fc.assert(
			fc.property(bufferGen(3, 5), async (smallBuffer) => {
				// Render small buffer
