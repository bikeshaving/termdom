...
import {inspectElement, inspectDocument, inspectNode, setupInspectMethods} from "../src/inspector.js";
...
test("inspectElement compact mode", () => {
	const dom = new JSDOM(`<div><span>A</span><span>B</span><span>C</span></div>`);
	const div = dom.window.document.querySelector("div");
...
test("inspectElement shows important attributes", () => {
	const dom = new JSDOM(`<input type="text" name="username" value="john" placeholder="Enter name">`);
...
test("inspectElement truncates long text", () => {
	const longText = "This is a very long text that should be truncated when displayed in the inspector output to avoid making it too verbose";
	const dom = new JSDOM(`<p>${longText}</p>`);
	const p = dom.window.document.querySelector("p");
	
	const output = inspectElement(p!, { colorize: false });
	
	expect(output).toContain('<p>This is a very long text that should be truncat...</p>');
});

test("inspectDocument includes doctype and root", () => {
	const dom = new JSDOM(`<!DOCTYPE html><html><head><title>Test</title></head><body>Hello</body></html>`);
	
	const output = inspectDocument(dom.window.document, { colorize: false, maxDepth: 2 });
...
test("setupInspectMethods adds inspect to prototypes", () => {
	...
	expect(typeof div[inspect]).toBe('function');
	
	// Test that it works
	const output = div[inspect](2, { colors: false });
	expect(output).toBe('<div id="test">Hello</div>');
});

test("showStyles option includes style attribute", () => {
	const dom = new JSDOM(`<div style="color: red; background: blue;">Styled</div>`);
	const div = dom.window.document.querySelector("div");
	...
	const withoutStyles = inspectElement(div!, { colorize: false, showStyles: false });
	expect(withoutStyles).not.toContain('style=');
});

test("showAll option includes all attributes", () => {
	const dom = new JSDOM(`<div id="test" data-foo="bar" aria-label="Test" custom="value">Content</div>`);
	const div = dom.window.document.querySelector("div");
