...
import {breakNodes} from "./TextBreaker.js";
...
test("TextBreaker - text wrapping", () => {
	const breaker = new TextBreaker();
	const result = breaker.breakNodes([textNode("Hello beautiful world")], {
		maxWidth: 10,
	});
...
test("TextBreaker - whitespace normal", () => {
	const breaker = new TextBreaker();
	const result = breaker.breakNodes([textNode("Hello   world   ")], {
		maxWidth: 20,
		whiteSpace: "normal",
	});
...
test("TextBreaker - whitespace pre", () => {
	...
	const text = result.lines[0].nodes
		.map((n) => {
...
test("TextBreaker - multiple text nodes", () => {
	const breaker = new TextBreaker();
	const result = breaker.breakNodes(
		[textNode("Hello "), textNode("beautiful "), textNode("world")],
		{maxWidth: 15},
	);
	...
	const nodeCount = result.lines.reduce(
		(sum, line) => sum + line.nodes.length,
		0,
	);
...
test("TextBreaker - character positions", () => {
	const breaker = new TextBreaker();
	const nodes = [textNode("Hello "), textNode("world")];
...
test("TextBreaker - pre-line whitespace", () => {
	const breaker = new TextBreaker();
	const result = breaker.breakNodes([textNode("Line 1  \nLine 2  ")], {
		maxWidth: 100,
		whiteSpace: "pre-line",
	});
...
test("TextBreaker - break-word", () => {
	const breaker = new TextBreaker();
	const result = breaker.breakNodes([textNode("verylongwordthatdoesntfit")], {
		maxWidth: 10,
