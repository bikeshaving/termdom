...
// Wait for the first frame to paint before measuring. The flow-or-pager
// decision below compares the document's height to the window's, and a
// transport that settles its size after attach (the browser playground's
// does) reports the real geometry only after the first frame. A frame
// callback runs before its frame paints, so the second one is after.
