...
function getRootRetainer(renderer3, bridge, { children, root, hydrate: hydrate2 }) {
  let ret;
...
function commitHost(adapter2, ret, ctx, root, schedulePromises, hydrationNodes) {
  ...
  if (!copyChildren) {
    const children = commitChildren(adapter2, ret, ctx, scope, tag === Portal ? node2 : root, ret, 0, schedulePromises, hydrationMetaProp && !hydrationMetaProp.includes("children") ? void 0 : childHydrationNodes);
    adapter2.arrange({
      tag,
      tagName: getTagName(tag),
      node: node2,
  ...
  for (let s = 0; s < spans.length; s++) {
    ...
    for (let i = 0, end = i; i < span.length; i = end) {
      ...
      switch (matcher) {
            ...
            if (comment2) {
              ...
              if (closingSlash) {
                element.close = {
                  ...
                  value: tagName,
                  spanIndex: s,
                  charIndex: match2.index
                };
                if (!stack.length) {
                  if (end !== span.length) {
                    throw new SyntaxError(formatSyntaxError(`Unmatched closing tag "${tagName}"`, spans, s, match2.index));
                  }
                  expressionTarget = {
                    type: "error",
        ...
        case PROPS_RE: {
          if (match2) {
            const [, tagEnd, spread, name, equals, string] = match2;
            if (i < match2.index) {
              throw new SyntaxError(formatSyntaxError(`Unexpected text \`${span.slice(i, match2.index).trim()}\``, spans, s, i));
            }
            if (tagEnd) {
              if (tagEnd[0] === "/") {
                element = stack.pop();
              ...
              if (!(expressing && end === span.length)) {
                throw new SyntaxError(formatSyntaxError('Expression expected after "..."', spans, s, match2.index));
              }
            } else if (name) {
              let value;
              if (string == null) {
                if (!equals) {
                  ...
                  expressionTarget = value;
                  if (!(expressing && end === span.length)) {
                    throw new SyntaxError(formatSyntaxError(`Expression expected for prop "${name}"`, spans, s, match2.index));
                  }
                }
              } else {
...
`;
  const start = Math.max(0, line2 - 1);
  const end = Math.min(lines.length - 1, line2 + 1);
...
var writablePropertyCache = /* @__PURE__ */ new WeakMap();
function isWritableProperty(element, name) {
  if (Object.prototype.hasOwnProperty.call(element, name)) {
    const descriptor = Object.getOwnPropertyDescriptor(element, name);
    return descriptor != null && (descriptor.writable === true || descriptor.set !== void 0);
  }
  ...
  if (propOwner !== null) {
    ...
    result = descriptor != null && (descriptor.writable === true || descriptor.set !== void 0);
  }
  if (ctorCache === void 0) {
...
var adapter = {
  ...
  adopt({ tag, tagName, node: node2, root }) {
    if (typeof tag !== "string" && tag !== Portal) {
      throw new Error(`Unknown tag: ${tagName}`);
    }
    const doc = getRootDocument(root);
    if (node2 === doc.body || node2 === doc.head || node2 === doc.documentElement || node2 === doc) {
  ...
  patch({ tag, tagName, node: node2, props, oldProps, scope: xmlns, copyProps, quietProps, isHydrating }) {
...
var impl = {
  ...
  arrange({ tag, tagName, node: node2, props, children, scope }) {
    var _a;
    if (tag === Portal) {
      return;
...
var BG_DARK = "#0d1117";
var BG_LIGHT = "#f7f9fb";
var TEXT_DARK = "#e6edf3";
var TEXT_LIGHT = "#10151c";
function useColorScheme(ctx) {
  ...
  const onStorage = (e) => {
    if (e.key === "color-scheme" && e.storageArea === sessionStorage && (e.newValue === "dark" || e.newValue === "light")) {
      current = e.newValue;
      applyColorScheme(current);
      ctx.refresh();
    }
    ...
    yield jsx`
			...
			>
				...
				<span
					hydrate="!style"
					class=${knobStyles}
					style=${IS_CLIENT ? { display: "block", left: isDark ? "28px" : "2px" } : void 0}
				/>
			</button>
		`;
  }
