...
async function main(): Promise<void> {
	...
	for (const probe of probes) {
		const {supported, note} = await probe.run();
		results.push({probe, supported, note});
	}

	// Properties in the platform that we never probe at all: named, so the gap
	// between "what exists" and "what we checked" is visible rather than implied.
	const known = new Set(probes.map((p) => p.name));
	const unprobed = Object.keys(properties)
		.filter(
			(p) =>
				!p.startsWith("-") &&
	...
	const lines: string[] = [
		...
		"",
		`Probed ${results.length} features against the CSS property list from`,
		`\`mdn-data\`; ${supported} have an observable effect.`,
		"",
	];
	...
	lines.push(
		"## Not probed",
		"",
		`${unprobed.length} standard CSS properties have no probe here, either because`,
	...
	process.stdout.write(
		`Wrote SUPPORT.md: ${supported}/${results.length} probed features supported, ` +
			`${unprobed.length} unprobed.\n`,
	);
}
