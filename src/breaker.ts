...
import {createExpandedTreeWalker} from "./composition.js";
...
function collectLeafNodes(runHead: Node): Leaf[] {
	...
	const window = runHead.ownerDocument?.defaultView as unknown as DOMWindow;
	if (!window) {
		throw new Error("No window available for TreeWalker");
	}
