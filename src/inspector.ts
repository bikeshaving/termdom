 ...
 */
export function inspectElement(element: Element, options: InspectorOptions = {}): string {
 ...
 */
export function inspectDocument(doc: Document, options: InspectorOptions = {}): string {
	const { colorize = true } = options;
	const c = colorize ? colors : { tag: '', attr: '', value: '', text: '', comment: '', reset: '', dim: '', bold: '' };
 ...
 */
export function inspectNode(node: Node, options: InspectorOptions = {}): string {
 ...
 */
function formatElement(element: Element, options: InspectorOptions & { currentDepth?: number } = {}): string {
	const { maxDepth = 2, colorize = true, compact = false, showStyles = false, showAll = false, currentDepth = 0 } = options;
	const c = colorize ? colors : { tag: '', attr: '', value: '', text: '', comment: '', reset: '', dim: '', bold: '' };
	...
	if (currentDepth < maxDepth && element.childNodes.length > 0) {
		const childrenStr = formatChildren(element, { ...options, currentDepth: currentDepth + 1 });
 ...
 */
function formatAttributes(element: Element, options: { colorize?: boolean; showAll?: boolean; showStyles?: boolean } = {}): string {
	...
	if (element.className) {
		attrs.push(`${c.attr}class${c.reset}=${c.value}"${element.className}"${c.reset}`);
	}
	...
	if (showAll) {
		...
		for (const attr of Array.from(element.attributes)) {
			if (attr.name !== 'id' && attr.name !== 'class' && attr.name !== 'style') {
				attrs.push(`${c.attr}${attr.name}${c.reset}=${c.value}"${attr.value}"${c.reset}`);
			}
		}
	} else {
		// Show only important attributes
		const importantAttrs = ['href', 'src', 'type', 'name', 'value', 'disabled', 'checked', 'selected', 'readonly', 'placeholder', 'alt', 'title'];
		for (const attrName of importantAttrs) {
			if (element.hasAttribute(attrName)) {
				const value = element.getAttribute(attrName);
				if (value !== null && value.length < 50) {
					attrs.push(`${c.attr}${attrName}${c.reset}=${c.value}"${value}"${c.reset}`);
				}
			}
		}
	}
	...
	if (showStyles && element.hasAttribute('style')) {
		...
		if (style && style.length < 100) {
			...
			attrs.push(`${c.attr}style${c.reset}=${c.value}"${style.substring(0, 97)}..."${c.reset}`);
		}
	}
 ...
 */
function formatChildren(element: Element, options: InspectorOptions & { currentDepth?: number }): string {
 ...
 */
function inspectComment(comment: Comment, options: InspectorOptions = {}): string {
	const { colorize = true } = options;
	const c = colorize ? colors : { tag: '', attr: '', value: '', text: '', comment: '', reset: '', dim: '', bold: '' };
 ...
 */
function inspectFragment(fragment: DocumentFragment, options: InspectorOptions = {}): string {
	const { colorize = true } = options;
	const c = colorize ? colors : { tag: '', attr: '', value: '', text: '', comment: '', reset: '', dim: '', bold: '' };
 ...
 */
export function inspectCSSStyleDeclaration(styles: any, options: InspectorOptions = {}): string {
	const { colorize = true, compact = false } = options;
	const c = colorize ? colors : { tag: '', attr: '', value: '', text: '', comment: '', reset: '', dim: '', bold: '' };
 ...
 */
export function inspectNodeList(nodeList: any, options: InspectorOptions = {}): string {
	...
	if (maxDepth === 0) {
		...
		const preview = Array.from(nodeList).slice(0, 3).map((node: any) => {
			if (!node) return 'null';
...
export function setupInspectMethods(window: any): void {
	...
	if (!window.Element.prototype[inspect]) {
		window.Element.prototype[inspect] = function(this: Element, depth: number, options: any) {
	...
	if (!window.Document.prototype[inspect]) {
		window.Document.prototype[inspect] = function(this: Document, depth: number, options: any) {
	...
	if (!window.Text.prototype[inspect]) {
		window.Text.prototype[inspect] = function(this: Text, depth: number, options: any) {
	...
	if (!window.Comment.prototype[inspect]) {
		window.Comment.prototype[inspect] = function(this: Comment, depth: number, options: any) {
	...
	if (!window.DocumentFragment.prototype[inspect]) {
		window.DocumentFragment.prototype[inspect] = function(this: DocumentFragment, depth: number, options: any) {
	...
	if (window.CSSStyleDeclaration && !window.CSSStyleDeclaration.prototype[inspect]) {
		window.CSSStyleDeclaration.prototype[inspect] = function(this: any, depth: number, options: any) {
	...
	if (window.NodeList && !window.NodeList.prototype[inspect]) {
		window.NodeList.prototype[inspect] = function(this: any, depth: number, options: any) {
			const isNested = depth < 2;
			const explicitlyCompact = options.compact;
			const hasBreakLength = options.breakLength && options.breakLength !== Infinity;
	...
	if (window.HTMLCollection && !window.HTMLCollection.prototype[inspect]) {
		window.HTMLCollection.prototype[inspect] = function(this: any, depth: number, options: any) {
			const isNested = depth < 2;
			const explicitlyCompact = options.compact;
			const hasBreakLength = options.breakLength && options.breakLength !== Infinity;
