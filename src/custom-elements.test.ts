...
test("custom element with anonymous slot", () => {
	...
	termdom.window.customElements.define("my-wrapper", MyWrapper);
	...
	const mergedTree = (termdom as any).createMergedDOMTree(shadowRoot, myWrapper);
...
test("custom element with named slots", () => {
	...
	termdom.window.customElements.define("my-card", MyCard);
	...
	expect((mergedHeader.childNodes[1] as Element).textContent).toBe("Custom Card Title");
	...
	expect((mergedBody.childNodes[1] as Element).textContent).toBe("Main content paragraph 1");
	expect((mergedBody.childNodes[1] as Element).tagName).toBe("P");
	expect((mergedBody.childNodes[2] as Element).textContent).toBe("Main content paragraph 2");
	...
	expect((mergedFooter.childNodes[1] as Element).textContent).toBe("Custom Footer Text");
...
test("custom element fallback content when no light DOM", () => {
	...
	termdom.window.customElements.define("my-button", MyButton);
...
test("light DOM changes after initial render", () => {
	...
	termdom.window.customElements.define("dynamic-wrapper", DynamicWrapper);
	const element = document.createElement("dynamic-wrapper") as any;

	// Initial state - should use fallback
	let mergedTree = (termdom as any).createMergedDOMTree(element.shadowRoot, element);
	...
	mergedTree = (termdom as any).createMergedDOMTree(element.shadowRoot, element);
...
test("shadow DOM structure changes", () => {
	...
	termdom.window.customElements.define("modifiable-shadow", ModifiableShadow);
...
test("slot attribute changes on light DOM", () => {
	...
	termdom.window.customElements.define("slot-switcher", SlotSwitcher);
...
test("multiple elements with same slot name", () => {
	...
	termdom.window.customElements.define("multi-slot", MultiSlot);
	...
	const mergedTree = (termdom as any).createMergedDOMTree(element.shadowRoot, element);
...
test("empty slots and edge cases", () => {
	...
	termdom.window.customElements.define("edge-case", EdgeCase);
...
test("text nodes vs elements in slots", () => {
	...
	termdom.window.customElements.define("mixed-content", MixedContent);
