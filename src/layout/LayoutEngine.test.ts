...
import { LayoutEngine, isInlineRunHead, findInlineRunHead } from "./LayoutEngine.js";
...
test("styleYogaNode - basic layout", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="width: 100px; height: 50px;"></div>`);
...
test("styleYogaNode - percentage dimensions", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="width: 50%;"></div>`);
	const div = jsdom.window.document.querySelector("div")!;
	const rect = layoutEngine.getRect(div);
...
test("styleYogaNode - margins", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="margin: 10px;"></div>`);
	const div = jsdom.window.document.querySelector("div")!;
	const rect = layoutEngine.getRect(div);
...
test("style changes trigger layout updates", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="width: 100px;"></div>`);
	const div = jsdom.window.document.querySelector("div")!;
...
test("element removal cleans up yoga nodes", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div><span>test</span></div>`);
	const div = jsdom.window.document.querySelector("div")!;
...
test("display none elements", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="display: none;"></div>`);
	const div = jsdom.window.document.querySelector("div")!;
...
test("resize updates layout", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="width: 100%;"></div>`);
	const div = jsdom.window.document.querySelector("div")!;
...
test("isInlineRunHead - multiple inlines in flex container", () => {
	const dom = createDOM(`<div style="display: flex"><span>first</span><span>second</span></div>`);
	const spans = Array.from(dom.window.document.querySelectorAll("span"));
...
test("findInlineRunHead - mixed content", () => {
	const dom = createDOM(`Text <span>element</span> more text <em>emphasis</em>`);
...
test("findInlineRunHead - flex container text nodes", () => {
	const dom = createDOM(`<div style="display: flex">Text <span>element</span> more</div>`);
	const textNode = getTextNode(dom, "Text");
...
test("inline-block vs inline behavior", () => {
	const dom = createDOM(`<div><span style="display: inline">inline</span><span style="display: inline-block">inline-block</span></div>`);
	const spans = Array.from(dom.window.document.querySelectorAll("span"));
...
test("text node with inline precedent in flex container", () => {
	const dom = createDOM(`<div style="display: flex"><span>element</span> text content</div>`);
	const span = getNode(dom, "span");
	const textNode = getTextNode(dom, "text content");
...
test("block insertion splits inline run", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div><span>first</span><span>second</span></div>`);
...
test("inline head deletion promotes next element", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div><span>head</span><span>second</span><span>third</span></div>`);
	const spans = Array.from(jsdom.window.document.querySelectorAll("span"));
...
test("inline middle element deletion maintains run", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div><span>first</span><span>middle</span><span>last</span></div>`);
	const spans = Array.from(jsdom.window.document.querySelectorAll("span"));
...
test("text insertion in flex container creates new run", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="display: flex"><span>element</span></div>`);
	const container = getNode(jsdom, "div");
...
test("inline insertion between text nodes in flex", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="display: flex">First text</div>`);
	const container = getNode(jsdom, "div");
...
test("flex to block conversion changes run behavior", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div style="display: flex"><span>first</span><span>second</span></div>`);
	const container = getNode(jsdom, "div");
	const spans = Array.from(jsdom.window.document.querySelectorAll("span"));
...
test("text node removal affects inline run heads", () => {
	const { jsdom, layoutEngine } = createLayoutEngine(`<div>Text content<span>element</span></div>`);
