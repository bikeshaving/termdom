...
import {
	...
	installForwardedEventHandler,
	isConstructingInternally,
	isMouseEvent,
	KeyboardEvent,
	MessageEvent,
	MouseEvent,
...
import type {DispatchState, EventInit, Listener} from "./events.ts";
...
export function dispatchFromOutside(
	target: EventTarget,
	event: Event,
	trusted: boolean,
): boolean {
	...
	const state = getDispatchState(event);
...
export function dispatch(
	...
	if (target !== relatedTarget || target === state.relatedTarget) {
		...
		const isActivationEvent = isMouseEvent(event) && event.type === "click";
		...
		let parent = getTheParent(eventTarget, event);
		while (parent !== null) {
			if (slottable !== null) {
				slottable = null;
...
function invokeListeners(
	...
	const listeners = getListeners(struct.invocationTarget).slice();
...
function innerInvoke(
	...
	for (const listener of listeners) {
		...
		if (listener.once) {
			...
			removeListener(getListeners(target), listener);
		}
		if (listener.passive) {
...
function deliverToSession(exchange: EventTarget, event: Event): void {
	...
	const listeners = getListeners(exchange).slice();
...
export function getEventTargetDocument(target: EventTarget): Document | null {
...
// The spec's "get the parent" per target: a node returns its parent, an
// assigned slottable its slot, and a shadow root its host, except for an
// uncomposed event dispatched inside the tree, which ends there.
function getTheParent(target: EventTarget, event: Event): EventTarget | null {
	if (target instanceof ShadowRoot) {
		const path = getDispatchState(event).path;
		if (
			!event.composed &&
			path.length > 0 &&
			path[0].invocationTarget instanceof Node &&
			getRoot(path[0].invocationTarget) === target
		) {
			return null;
		}
		return target[kHost];
	...
	if (target instanceof Element || target instanceof Text) {
		return target[kAssignedSlot] ?? target[kParent];
	}
	if (target instanceof Node) {
		return target[kParent];
	}
	return null;
}
...
export class Element extends Node implements globalThis.Element {
	...
	getAttributeNode(qualifiedName: string): globalThis.Attr | null {
		return getAttributeByName(
			this,
			String(qualifiedName),
		) as unknown as globalThis.Attr | null;
	}
...
function buildElement(
	...
	let element: Element;
	try {
		element = constructInternal(() => new constructor());
	} finally {
...
class CustomElementRegistry {
	constructor() {
		this[kScoped] = !isConstructingInternally();
		this[kDefinitions] = [];
...
function getValidityState(element: Element): ValidityState {
	...
	if (state === undefined) {
		state = constructInternal(
			() => new ValidityState(() => getValidityFlags(element)),
		);
...
export class Document extends Node implements globalThis.Document {
	...
	createEvent(interfaceName: string): globalThis.Event {
		...
		getDispatchState(event).initialized = false;
	...
	open(_url: string |
		URL, _name: string, _features: string): globalThis.WindowProxy | null;
...
function walkChildren(walk: TreeWalker, first: boolean): Node | null {
	let node: Node | null =
		first ? walk[kCurrent][kFirstChild] : walk[kCurrent][kLastChild];
	while (node !== null) {
...
function frameSettled(document: Document, attached: AttachedDocument): Promise<
	void> {
