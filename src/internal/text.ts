...
// Bun has a native stringWidth; Node and Deno take the fallback.
...
// Two generations: a hit in the young one moves nothing; when it fills it
// becomes the old one and the previous old generation drops. At most twice
...
// (Terminal.app advances ☀️ one cell and ⛅️ two; no rule separates them).
// Not a cache: a measurement of the one terminal, written once, never stale.
const clusterAdvances = new Map<string, number>();

// Whether the ledger changed: the first answer stands, and agreement with
// the tables is not stored.
...
// holding none of it: two compares keep a screenful of ideographs off the regex.
...
// (selectors, ZWJ, flags, modifiers, marks), East Asian Ambiguous outside
// the scripts not in doubt, default emoji presentation, and Arabic
// presentation forms, which a ligating terminal advances differently. Runs
// once per painted cell and allocates nothing.
...
// zero, so a cumulative array answers any range at cluster boundaries.
...
// Must agree with Bun.stringWidth wherever Bun is still consulted, or text
// misrenders on Node and Deno only.
...
function graphemeWidth(cluster: string): number {
	...
	// After, so a cluster led by a bare U+FE0F does not promote itself.
	...
	// Every spacing mark after the base adds a cell: "का" occupies two. After
	// the base only, or a lone spacing mark is charged twice.
...
// Terminals mostly do not implement UAX #9, and one that does reorders a
// line as it arrives, which a diff of single cells cannot survive: cells go
// out in visual order (see negotiateBidi). bidi-js is UAX #9 entire;
// arabic-persian-reshaper picks the contextual letterforms reordering alone
// would leave isolated.
const {ArabicShaper} = arabicPersianReshaper;

const bidi = bidiFactory();

// Escapes, not literals: U+FEFF reads as stray whitespace in an editor.
const ARABIC =
	/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/;
...
// Shaping is not length-preserving (lam-alef becomes one code point), so it
// runs here on the painted line, never on the measured text: a gap beats a
// wrong caret.
export function toVisualOrder(text: string, base: "ltr" | "rtl"): string {
	...
	// L2 and L4: a terminal will not mirror a glyph, so the code point changes.
